  ...
  ipcMain.handle('tq-api', async (_event, method: string, payload: unknown) => {
    switch (method) {
      ...
      case 'exports:fromQuestions': {
        ...
        const exportId = String(body.exportId ?? `exp_${Date.now()}`)
        console.error('[PDF_EXPORT:IPC]', {
          exportId,
          file: 'electron/ipc-handlers.ts',
          case: 'exports:fromQuestions',
        })
        const enriched = exportService.enrichExportPayload({ ...body, exportId })
        const { bytes, diagnostics } = await exportService.exportPdf(enriched)
        return {
