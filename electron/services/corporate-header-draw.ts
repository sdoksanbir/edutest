...
export async function drawCorporateHeader(
  ...
  if (rightSlots.length > 0) {
    ...
    for (const { slot, top: topFromBody, h: slotH } of placed) {
      ...
      if (slot === 'examType' && examTypeLineSpecs(badge).length > 0) {
        const boxW = style1RightSlotWidthPt(slot, badge, maxBoxWPt)
        const boxX = geom.page_w_pt - geom.mr - boxW - resolveExamTypeOffsetXPt(badge)
        drawExamTypeBoxFillPdf(page, boxX, slotBottom, boxW, slotH, badge)
        drawExamTypeBoxBorderPdf(page, boxX, slotBottom, boxW, slotH, badge)
        drawExamTypeTextInBoxPdf(page, fonts.bold, boxX, slotBottom, boxW, slotH, badge)
      } else if (slot === 'score') {
        const boxW = style1RightSlotWidthPt(slot, badge)
        const boxX = geom.page_w_pt - geom.mr - boxW
        drawStyle1ScoreBoxPdf(page, fonts.bold, boxX, slotBottom, boxW, slotH, badge)
      } else if (slot === 'testNo') {
        drawStyle1TestNoPdf(
          page,
          fonts.bold,
...
function drawCenterTextBlock(
  ...
  const topicSubTopicGap = config.topicSubTopicGapPt ?? 3
  const topicBlockH = (() => {
