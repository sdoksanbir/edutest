...
import type { HeaderConfig, HeaderFieldFontSizesPt } from './corporateHeaderLayout'
import { normalizeHeaderStyleId } from './headerStyleIds'
...
export const HEADER_FONT_FIELD_KEYS: HeaderFontFieldKey[] = [
  'subject',
  'examType',
  'topic',
  'subTopic',
...
export type { HeaderFieldFontSizesPt }
...
const STYLE_1_DEFAULTS: Record<HeaderFontFieldKey, number> = {
  subject: CORPORATE_FONT_SUBJECT_PT,
  examType: CORPORATE_FONT_EXAM_PT,
  topic: CORPORATE_FONT_TOPIC_PT,
  subTopic: CORPORATE_FONT_TOPIC_PT,
  authorName: CORPORATE_FONT_AUTHOR_PT,
  brandName: CORPORATE_FONT_BRAND_PT,
  schoolName: 7,
  testNumber: 8,
  testType: 7,
  publisherLine: CORPORATE_FONT_PUBLISHER_MAIN_PT,
}
...
/** Konu varsayılan kalın; alt konu varsayılan normal */
export function headerFieldBold(
  config: HeaderConfig,
  field: HeaderFontFieldKey,
  defaultBold = false,
): boolean {
  const v = config.fieldFontStyles?.[field]?.bold
  return typeof v === 'boolean' ? v : defaultBold
}

export function headerFieldItalic(
  config: HeaderConfig,
  field: HeaderFontFieldKey,
  defaultItalic = false,
): boolean {
  const v = config.fieldFontStyles?.[field]?.italic
  return typeof v === 'boolean' ? v : defaultItalic
}

export function canvasFontStyleCss(bold: boolean, italic: boolean, sizePx: number, family: string): string {
...
export function parseHeaderFieldFontSizesPt(raw: unknown): HeaderFieldFontSizesPt {
  ...
  for (const key of HEADER_FONT_FIELD_KEYS) {
    const v = o[key]
    if (v == null) continue
    const n = Number(v)
...
export function formatHeaderFieldFontPtLabel(pt: number): string {
  const text = Number.isInteger(pt) ? String(pt) : pt.toFixed(1)
  return `${text}pt`
}
...
/** Kısa alias — canvas / PDF çiziminde */
