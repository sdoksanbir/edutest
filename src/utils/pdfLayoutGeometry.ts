...
import { resolveClassicBannerAndInfoHeightPt } from "./bannerRightMode";
...
function classicBannerAndInfoHeightPt(input: LayoutGeometryInput): number {
  ...
  const cfg = {
    ...mergeHeaderBadgeConfig(input.headerConfig, input.headerStyleId),
    showClassicInfoBar: input.headerConfig.showClassicInfoBar !== false,
  }
  const contentW =
    input.pageWpt - mmToPdfPt(input.marginLeftMm) - mmToPdfPt(input.marginRightMm)
  return resolveClassicBannerAndInfoHeightPt(cfg, input.headerStyleId, contentW)
}
...
export type LayoutGeometryInput = {
  ...
  /** Deneme: yönerge üstü meta satır yüksekliği */
  trialDescriptionMetaBar?: boolean;
...
function contentTopFirstBasePt(input: LayoutGeometryInput): number {
    ...
    const headerBlock = descriptionHeaderBlockHeightPt(
      {
        ...
        descriptionBoxPadYPt: input.descriptionBoxPadYPt,
        descriptionBoxPadXPt: input.descriptionBoxPadXPt,
        trialMetaBar: Boolean(input.trialDescriptionMetaBar),
      },
      contentW,
      undefined,
      classicBannerH,
  ...
  // Bilgi şeridi kapalıyken eski şerit altı DESC boşluğunu bırakma
  const gapBelow =
    input.headerConfig != null && input.headerConfig.showClassicInfoBar === false
      ? 0
      : DESC_BOX_GAP_BELOW_PT;
  return pageHpt - mt - classicBannerH - gapBelow;
}
