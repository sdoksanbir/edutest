...
export const EXAM_TYPE_BOX_MANUAL_DEFAULT_W_PT = STYLE_1_TEST_NO_W_DEFAULT_PT
export const EXAM_TYPE_BOX_MANUAL_MIN_H_PT = 16
export const EXAM_TYPE_BOX_MANUAL_MAX_H_PT = 72
export const EXAM_TYPE_BOX_MANUAL_DEFAULT_H_PT = 22
...
export const EXAM_TYPE_LINE1_FONT_DEFAULT_PT = 11
/** Ders adı (2. satır) varsayılan punto */
export const EXAM_TYPE_LINE2_FONT_DEFAULT_PT = 10
export const EXAM_TYPE_OFFSET_X_DEFAULT_PT = 0
export const EXAM_TYPE_OFFSET_X_MIN_PT = -48
export const EXAM_TYPE_OFFSET_X_MAX_PT = 48
export const EXAM_TYPE_OFFSET_Y_DEFAULT_PT = 7
export const EXAM_TYPE_OFFSET_Y_MIN_PT = -28
export const EXAM_TYPE_OFFSET_Y_MAX_PT = 28
...
export function examTypeLineSpecs(config: HeaderConfig): PublicationLineSpec[] {
  const slots = (config as HeaderConfig & { bannerRightSlots?: unknown }).bannerRightSlots
  if (Array.isArray(slots)) {
    if (!slots.includes('examType')) return []
  } else if (
    ...
    config.bannerRightMode !== 'examType' &&
    !isHeaderFieldVisible(config, 'examType')
  ) {
    return []
  }
...
/** Slider ile ayarlanan kutu genişliği (pt) — varsayılan: Test No toplam genişliği */
export function resolveExamTypeBoxWidthPt(
  config: HeaderConfig,
  maxAvailablePt?: number,
): number {
  let w = clampExamTypeBoxManualWidthPt(
    config.examTypeBoxManualWidthPt ?? resolveTestNoWidthPt(config),
  )
  if (maxAvailablePt != null && maxAvailablePt > 0) {
    w = Math.min(w, maxAvailablePt)
  }
...
export function clampExamTypeOffsetXPt(pt: number): number {
  const n = Number.isFinite(pt) ? pt : EXAM_TYPE_OFFSET_X_DEFAULT_PT
  return Math.max(
    EXAM_TYPE_OFFSET_X_MIN_PT,
    Math.min(EXAM_TYPE_OFFSET_X_MAX_PT, Math.round(n)),
  )
}

export function clampExamTypeOffsetYPt(pt: number): number {
  const n = Number.isFinite(pt) ? pt : EXAM_TYPE_OFFSET_Y_DEFAULT_PT
  return Math.max(
    EXAM_TYPE_OFFSET_Y_MIN_PT,
    Math.min(EXAM_TYPE_OFFSET_Y_MAX_PT, Math.round(n)),
  )
}

/** + değer kutuyu sola kaydırır (sağ kenardan içeri) */
export function resolveExamTypeOffsetXPt(config: HeaderConfig): number {
  return clampExamTypeOffsetXPt(config.examTypeOffsetXPt ?? EXAM_TYPE_OFFSET_X_DEFAULT_PT)
}

/** + değer kutuyu aşağı kaydırır */
export function resolveExamTypeOffsetYPt(config: HeaderConfig): number {
  return clampExamTypeOffsetYPt(config.examTypeOffsetYPt ?? EXAM_TYPE_OFFSET_Y_DEFAULT_PT)
}
...
export function examTypeBoxFillColor(config: HeaderConfig): string {
  return (config.examTypeBoxFillColor || '#0A1931').trim()
}
