...
import {
  columnContentRectsPxExcludingFullWidth,
...
import { retainQuestionImages } from "../../utils/questionImageCache";
...
import OptikFormDragOverlay from "../pdf/OptikFormDragOverlay";
...
import {
  ...
  getBulkScaleRunId,
  logBulkScaleCommit,
  logBulkScaleExportPayload,
  logBulkScalePreview,
  logBulkScaleSessionStart,
  logBulkScaleStateReadback,
  pendingFromBulkApply,
  pendingFromRequestedProduct,
  storeUpdateFromBulkApply,
  storeUpdateFromPending,
  type BulkPendingScale,
  type BulkScaleBaseline,
...
function previewSharpnessForQuality(q: "normal" | "high" | "best"): number {
  // DPR tavanı + sharpness; normal = ekran minimumu (bellek dostu)
  if (q === "best") return 1.5;
...
function PdfPreviewModalContent({ isOpen, onClose, variant = "test" }: PdfPreviewModalProps) {
  ...
  const allStoreQuestions = useEditorStore((s) => s.questions);
  /** Yazılı sorular yalnızca yazılı önizlemede; diğer modüllerde gizlenir */
  const questions = useMemo(
    () =>
      isWritten
        ? allStoreQuestions.filter((q) => Boolean(q.writtenType))
        : allStoreQuestions.filter((q) => !q.writtenType),
    [allStoreQuestions, isWritten],
  );
  ...
  const trialDescriptionMeta = useMemo(
    () =>
      isTrial
        ? {
            examCode: trialExamCode || "",
            testName: testName || "",
            bookletLabel: trialBookletLabel || "A KİTAPÇIĞI",
            examCodeFontPt: trialExamCodeFontPt ?? 10,
            examCodeColor: trialExamCodeColor || "",
            examCodeAlign: trialExamCodeAlign ?? "left",
            examCodePadLeftPt: trialExamCodePadLeftPt ?? 10,
            bookletFontPt: trialBookletFontPt ?? 10,
            bookletColor: trialBookletColor || "",
            testNameBgOpacityPct: trialTestNameBgOpacityPct ?? 100,
            testNameBgColor: trialTestNameBgColor || "#0A1931",
          }
        : null,
    [
      isTrial,
  ...
  useEffect(() => {
    if (!isOpen || !isTrial) {
      if (!isOpen) trialYonergeInitRef.current = false;
      return;
    }
    if (trialYonergeInitRef.current) return;
  ...
  const optikFormOffsetYPt = useEditorStore((s) => s.optikFormOffsetYPt);
  const setOptikFormOffsetYPt = useEditorStore((s) => s.setOptikFormOffsetYPt);
  ...
  const [sectionModalRange, setSectionModalRange] = useState<{
    start: number;
    end: number;
  } | null>(null);
  ...
  const applyPendingDisplayScales = useCallback((qs: QuestionItem[]) => {
    ...
    return qs.map((x) => {
      ...
      return {
        ...x,
        display_scale: p.requestedScale,
        manualScale: p.manualScale,
        // normalizationScale dokunulmaz
      };
    });
  }, []);

  const pendingRequestedScale = useCallback((questionId: string, fallback: number) => {
    return pendingDisplayScaleRef.current[questionId]?.requestedScale ?? fallback;
  }, []);
  ...
  const questionLayout = layout.filter((l) => l.kind !== "answer_key_page");
  const maxQuestionPage =
    questionLayout.length > 0
      ? Math.max(...questionLayout.map((l) => l.page_num ?? 1))
      : 1;
  ...
  /** Sayfada hem geniş hem dar soru varsa hibrit — sütun tuşu / overlay kapalı. */
  const hybridPageNums = useMemo(() => {
    const modeByOrder = new Map(
      questions.map((q) => [q.order_index, q.layoutMode] as const),
    );
    const pages = new Set<number>();
    for (const it of layout) {
      if (typeof it.page_num === "number" && it.kind !== "answer_key_page") {
        pages.add(it.page_num);
      }
    }
    const hybrid = new Set<number>();
    for (const pageNum of pages) {
      const band = computePageColumnBand({ ...layoutGeometryInput, pageNum });
      let hasFw = false;
      let hasNarrow = false;
      for (const it of layout) {
        if (it.page_num !== pageNum || it.kind === "answer_key_page") continue;
        if (
          isLayoutItemFullWidth(it, {
            questionLayoutMode: modeByOrder.get(it.order_index),
        ...
        if (hasFw && hasNarrow) break;
      }
      if (hasFw && hasNarrow) hybrid.add(pageNum);
    }
    return hybrid;
  }, [layout, questions, layoutGeometryInput]);

  const currentPageIsHybrid = hybridPageNums.has(currentPage);
  ...
  const optikDisplayNumberByOrder = useMemo(() => {
    const m = new Map<number, number | null>();
    for (const item of layout) {
      if (item.display_number != null) {
        m.set(item.order_index, item.display_number);
      }
    }
    return m;
  }, [layout]);
  ...
  const handleQuestionYTopChange = useCallback(
    (
      ...
      const shifted = applyImgYTopToLayoutItem(layoutRef.current, orderIndex, finalY);
    ...
    (pageNum: number) => {
      ...
      const layoutSrc =
        layoutLiveRef.current && layoutLiveRef.current.length > 0
          ...
          : layout;
      return columnContentRectsPxExcludingFullWidth({
        band,
        pageHpt,
        canvasScale: PREVIEW_PT_TO_PX * zoom,
        pageNum,
        layout: layoutSrc,
        questionLayoutModeByOrder: modeByOrder,
        contentTopByColumn: tops,
      });
    },
  ...
  const columnPanelItems = useMemo(() => {
    ...
    return getColumnItemsSortedTopFirst(
      ...
      band,
      {
        questionLayoutModeByOrder: new Map(
          questions.map((q) => [q.order_index, q.layoutMode] as const),
        ),
      },
    );
  }, [columnPanel, columnRedistBaseLayout, layoutGeometryInput, questions]);
  ...
  const handleColumnOverlayPointerDown = useCallback(
    (pageNum: number, col0: number, clientX: number, clientY: number) => {
      if (hybridPageNums.has(pageNum)) return;
      ...
      const band = computePageColumnBand(gi);
      const modeByOrder = new Map(
        useEditorStore.getState().questions.map((q) => [q.order_index, q.layoutMode] as const),
      );
    ...
    [layout, layoutGeometryInput, hybridPageNums]
  );
  ...
  // Hibrit sayfada sütun dağıtımını kapat
  useEffect(() => {
    if (!currentPageIsHybrid) return;
    if (!columnAdjustEnabled && columnPanel == null) return;
    setColumnPanel(null);
    setColumnRedistBaseLayout(null);
    columnPanelOpenLayoutRef.current = null;
    setColumnPanelOrderIndices([]);
  ...
  }, [currentPageIsHybrid, columnAdjustEnabled, columnPanel]);
  ...
  const fetchLayout = useCallback(
    async (
      ...
      const qs = questionsForTab(qsOverride ?? questions, isWritten ? "written-paper" : "test-paper");
      ...
      const payload = {
        ...
        columns: Math.max(1, Number(exportState.columns) || 1),
        ...
        questions: qsEffective.map((q) => ({
          ...
          writtenType: q.writtenType,
          writtenStemHtml: q.writtenStemHtml,
          writtenAnswerArea: q.writtenAnswerArea,
          writtenAnswerLines: q.writtenAnswerLines,
          writtenPoints: q.writtenPoints,
        })),
        ...
        ...(isWritten
          ? {
              ...
              written_accent_color: exportState.writtenPaperUi?.accentColor || "#0D9488",
            }
          : {}),
      };
      ...
      // Faz 1: motor layout kaynağı — istemci kaydırması yok
      const layoutToSet = finalized;
  ...
  const handleColumnShift = useCallback(
    (orderIndex: number, direction: ColumnShiftDirection, options?: { force?: boolean }) => {
      ...
      // Ham motor layout tercih: önceki zorla sıkıştırmanın h_pt'si taşınmasın
      const base =
        baseLayoutRef.current.length > 0
          ? baseLayoutRef.current
      ...
      // Zorla taşıma sonrası otomatik geri çekme yerleşimi bozar — atla
      if (!forced) {
  ...
  /**
   * Faz 1 — tek kaynak: tema / başlık geometrisi değişince yerel kaydırma yok.
   * Canvas her zaman layout-engine (fetchLayout) sonucunu çizer.
   */
  const headerThemeFetchSigRef = useRef("");
  useEffect(() => {
    if (!isOpen) {
      headerThemeFetchSigRef.current = "";
      return;
    }
    ...
    const sig = [
      ...
      headerStyleId,
      showInfo ? "1" : "0",
      headerConfig.useYaprakBanner === true ? "1" : "0",
      headerConfig.useExamBanner === true ? "1" : "0",
      options.includeDescription ? "1" : "0",
    ...
    headerThemeEpoch,
    headerStyleId,
    headerConfig.showClassicInfoBar,
    headerConfig.useYaprakBanner,
    headerConfig.useExamBanner,
    headerConfig.logoSizePct,
    headerConfig.logoPadYPt,
    headerConfig.headerLeftMode,
  ...
  const applySingleQuestionScaleReflow = useCallback(
    async (orderIndex: number, sizePct: number) => {
      ...
      const baseScale =
        session?.orderIndex === orderIndex
          ? session.baseScale
          : pendingRequestedScale(q.id, resolveRequestedScale(q));
  ...
  const getDisplayScaleMaxPctForOrder = useCallback(
    (orderIndex: number): number => {
      ...
      const baseScale =
        session?.orderIndex === orderIndex
          ? session.baseScale
          : pendingRequestedScale(q?.id ?? "", q ? resolveRequestedScale(q) : 1);
  ...
  const buildBulkScaleEntries = useCallback(
    (orderIndices: number[]) => {
      ...
      for (const q of qs) {
        ...
        const norm = resolveNormalizationScale(q);
        const manual = resolveManualScale(q);
        const requested = resolveRequestedScale(q);
        const baseline = captureBulkScaleBaseline({
          questionId: q.id,
          questionNo: (baseItem.display_number ?? q.order_index + 1) as number,
          orderIndex: q.order_index,
          normalizationScale: norm,
          manualScale: manual,
          requestedScale: requested,
          imgWPt: baseItem.img_w_pt,
          imgHPt: baseItem.img_h_pt,
          layoutMode: baseItem.layout_mode ?? q.layoutMode ?? "single-column",
          scaleDiag: baseItem.scale_diag ?? null,
        });
        entries.push({
          orderIndex: q.order_index,
          questionId: q.id,
  ...
  const startAllQuestionsScaleSession = useCallback(() => {
    ...
    logBulkScaleSessionStart(entries.map((e) => e.baseline));
  ...
  const applyBulkScalePreview = useCallback(
    (pct: number, mode: "all" | "selected", setSlider: (value: number) => void) => {
      ...
      setSlider(clamped);
      const applyResults = session.entries.map((entry) =>
  ...
  const commitBulkScale = useCallback(
    (pct: number, mode: "all" | "selected", resetSlider: () => void) => {
      ...
      // Session cancel ile silinmiş olsa bile pending varsa store’a yaz
      if (!session || session.mode !== mode) {
        const pendingIds = Object.keys(pendingSnapshot);
        if (pendingIds.length > 0) {
          ...
          logBulkScaleStateReadback(
            after
              .filter((q) => updates[q.id])
              .map((q) => ({
                questionId: q.id,
                questionNo: q.order_index + 1,
                manualScale: resolveManualScale(q),
                normalizationScale: resolveNormalizationScale(q),
                display_scale: q.display_scale ?? resolveRequestedScale(q),
                resolvedRequestedScale: resolveRequestedScale(q),
              })),
          );
          resetSlider();
          clearLayoutYTopOverrides();
      ...
      const beforeById = new Map(
        useEditorStore.getState().questions.map((q) => [q.id, q] as const),
      );
      const updates: Record<string, BulkScaleStoreUpdate> = {};
      ...
      for (const entry of session.entries) {
        const result = applyBulkScaleToBaseline(entry.baseline, relativeFactor);
        const storeUpdate = storeUpdateFromBulkApply(entry.baseline, result);
        updates[entry.questionId] = storeUpdate;
        const prev = beforeById.get(entry.questionId);
        commitRows.push({
          questionId: entry.questionId,
          questionNo: entry.baseline.questionNo,
          normalizationScale: storeUpdate.normalizationScale,
          previousManualScale: prev ? resolveManualScale(prev) : entry.baseline.manualScale,
          newManualScale: storeUpdate.manualScale,
          previousDisplayScale: prev
            ? resolveRequestedScale(prev)
            : entry.baseline.requestedScale,
          newDisplayScale: storeUpdate.display_scale,
          desiredAppliedScale: result.desiredAppliedScale,
          desiredDrawWidth: result.resultingDrawWidth,
        });
      }

      logBulkScaleCommit(relativeFactor, commitRows);
      // Atomik: manualScale + normalizationScale + display_scale
      setQuestionsBulkScales(updates);
      ...
      const afterQs = useEditorStore.getState().questions;
      logBulkScaleStateReadback(
        afterQs
          .filter((q) => updates[q.id])
          .map((q) => ({
            questionId: q.id,
            questionNo: (commitRows.find((r) => r.questionId === q.id)?.questionNo ??
              q.order_index + 1) as number,
            manualScale: resolveManualScale(q),
            normalizationScale: resolveNormalizationScale(q),
            display_scale: q.display_scale ?? resolveRequestedScale(q),
            resolvedRequestedScale: resolveRequestedScale(q),
      ...
      void fetchLayout(undefined, useEditorStore.getState().questions, { silent: true }).then(
        () => {
          ...
          const again = useEditorStore.getState().questions.filter((q) => updates[q.id]);
          const reset = again.filter((q) => Math.abs(resolveManualScale(q) - 1) < 1e-9);
          if (reset.length > 0) {
            console.warn(
              `[ScaleDiag:BULK_SCALE_STATE_READBACK] bulkScaleRunId=${getBulkScaleRunId() ?? "none"} | ` +
                `fetchLayout sonrası manualScale=1 — layout refresh overwrite?`,
              reset.map((q) => q.id),
            );
          }
        },
    ...
    [setQuestionsBulkScales, fetchLayout, clearLayoutYTopOverrides],
  );
  ...
  // Seçili soru(lar): her zaman göreli çarpan (%100 = mevcut boyut) — tek soruda mutlak % karışıklığı yok
  ...
  const selectedFullWidthChecked = useMemo(() => {
    if (selectedQuestionOrders.length === 0) return false;
    const qs = questions.filter((q) => selectedQuestionOrders.includes(q.order_index));
    return qs.length > 0 && qs.every((q) => q.layoutMode === 'full-width');
  }, [questions, selectedQuestionOrders]);

  const layoutRecommendationHint = useMemo(() => {
    if (selectedQuestionOrders.length !== 1) return null;
    const order = selectedQuestionOrders[0]!;
    const item = layout.find((l) => l.order_index === order);
    const diag = item?.scale_diag;
    const fulfill = diag?.singleColumnFulfillment;
  ...
  const handleQuestionLayoutModeChange = useCallback(
    (orderIndex: number, mode: "single-column" | "full-width") => {
      const q = useEditorStore.getState().questions.find((x) => x.order_index === orderIndex);
      if (!q) return;
      // Hibrit: Geniş üst bant + Dar yan yana → en az 2 sütun gerekir
      if (useEditorStore.getState().columns < 2) {
        setColumns(2);
      }
  ...
  const handleSelectedFullWidthChange = useCallback(
    (fullWidth: boolean) => {
      const mode = fullWidth ? 'full-width' : 'single-column';
      if (useEditorStore.getState().columns < 2) {
        setColumns(2);
      }
      const qs = useEditorStore.getState().questions;
      for (const order of selectedQuestionOrders) {
        const q = qs.find((x) => x.order_index === order);
        if (q) setQuestionLayoutMode(q.id, mode);
      }
      clearLayoutYTopOverrides();
      void fetchLayout(undefined, useEditorStore.getState().questions, { silent: true }).then(() => {
        clearLayoutLivePreview();
  ...
  const selectedQuestionScaleMaxPct = useMemo(() => {
    ...
    const orderIndex = selectedQuestionOrders[0]!;
    const absMaxPct = getDisplayScaleMaxPctForOrder(orderIndex);
    const q = questions.find((x) => x.order_index === orderIndex);
    const curPct = Math.max(
      1,
      Math.round(
        pendingRequestedScale(q?.id ?? "", q ? resolveRequestedScale(q) : 1) * 100,
      ),
    );
    // Göreli slider: %100 = mevcut; üst sınır = sütun mutlak max / mevcut
    return Math.max(
      DISPLAY_SCALE_NEUTRAL_PCT,
      Math.min(DISPLAY_SCALE_MAX_PCT, Math.round((absMaxPct / curPct) * 100)),
    );
  }, [selectedQuestionOrders, getDisplayScaleMaxPctForOrder, layout, questions]);
  ...
  const buildLiveExportPayload = useCallback(() => {
    ...
    const s = useEditorStore.getState();
    const qsForExport = applyPendingDisplayScales(
      questionsForTab(s.questions, isWritten ? "written-paper" : "test-paper"),
    );
  ...
  const buildLiveExportPayloadAsync = useCallback(async () => {
    ...
    const previewLayout =
      layoutLiveRef.current && layoutLiveRef.current.length > 0
        ? layoutLiveRef.current
        : layoutRef.current;
    let optikOverlays: Awaited<ReturnType<typeof buildOptikFormPdfOverlays>> = [];
    if (s.optikFormEnabled && previewLayout.length > 0) {
      try {
        optikOverlays = await buildOptikFormPdfOverlays({
          enabled: s.optikFormEnabled,
          placement: s.optikFormPlacement,
          optionCount: s.optikFormOptionCount,
          bookletType: s.optikFormBookletType,
          netRule: s.optikFormNetRule,
          instructionEnabled: s.optikFormInstructionEnabled,
          instructionText: s.optikFormInstructionText,
          ...
          columns: s.columns,
          columnGapMm: 8,
          testTitle: s.testName?.trim() || "TEST",
          schoolName: s.schoolName?.trim() || "",
          includeAnswerKey: s.options.includeAnswerKey,
          answerKeyMode:
            (isWritten || isTrial || isFasikul) && s.options.includeAnswerKey
    ...
    return {
      ...payload,
      header_config: { ...hc, logoUrl },
      ...(optikOverlays.length > 0 ? { optik_form_overlays: optikOverlays } : {}),
    };
  }, [
  ...
  const runVerifyPdf = useCallback(async () => {
    ...
    try {
      ...
      const { blob, diagnostics } = await api.exports.fromQuestions({
        ...(await buildLiveExportPayloadAsync()),
        exportId,
      });
      console.error("[PDF_EXPORT:UI] diagnostics", diagnostics);
  ...
  const handleSavePdf = async () => {
    ...
    try {
      ...
      const { blob, diagnostics } = await api.exports.fromQuestions({
        ...(await buildLiveExportPayloadAsync()),
        exportId,
        bulk_scale_run_id: bulkScaleRunId,
      });
      console.error("[PDF_EXPORT:UI] export summary", {
        ...
        usedLockedLayoutPositions: diagnostics.usedLockedLayoutPositions,
        drawImageLogCount: diagnostics.drawImageLogCount,
        rendererFile: diagnostics.rendererFile,
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden pdf-preview-scroll">
        ...
        {!isWritten && !isTrial && (
          <SectionAddModal
            ...
            initialStartIdx={sectionModalRange?.start ?? null}
            initialEndIdx={sectionModalRange?.end ?? null}
          />
        )}
                      ...
                      >
                        ...
                        questionNumberFontPt={questionNumberFontPt}
                              ...
                              <OptikFormDragOverlay
                                ...
                                otherPageHeaderBottomGapMm={otherPageHeaderBottomGapMm}
                                writtenPaperHeader={isWritten}
                                onOffsetChange={(next, phase) => {
                                  setOptikFormOffsetYPt(next);
                                  if (phase === "commit" || phase === "move") {
                                    scheduleAllPreviewRedraw();
                                  }
                                }}
                              />
              ...
              <>
                ...
                <div className="pdf-preview-panel-optik pdf-preview-panel-optik-inner flex min-h-0 min-w-0 flex-1 overflow-hidden">
                  <OptikFormSidebar
                    ...
                    sections={sections}
                    displayNumberByOrder={optikDisplayNumberByOrder}
                    onReorder={applyQuestionReorder}
                    onQuestionNavigate={handleOptikQuestionNavigate}
                    onRequestSectionRange={(start, end) => {
                      setSelectedQuestionOrders(
                        Array.from({ length: end - start + 1 }, (_, i) => start + i),
                      );
                      setSelectedQuestion(start);
                      setSectionModalRange({ start, end });
                      setSectionModalOpen(true);
                    }}
                  />
                </div>
          ...
          >
            {showRightThemePanel ? (
              <>
                ...
                />
                <ThemeCustomizerSidebar variant={variant} />
              </>
            ) : (
              <PdfPreviewPanelRailButton
                variant="theme"
        ...
        {!loading && !error && layoutReady && layout.length > 0 ? (
          <div className="pdf-preview-bottom-actions flex items-center justify-center gap-1 justify-self-center rounded-lg border border-slate-200 bg-white/95 px-1.5 py-1 shadow-sm dark:border-[#30363d] dark:bg-[#21262d]">
            ...
            <button
              ...
              title={
                currentPageIsHybrid
                  ? "Bu sayfada geniş + dar (hibrit) soru var — sütun dağıtımı kapalı"
                  : "Sütun seçip dikey dağıtım"
              }
              disabled={currentPageIsHybrid}
              onClick={() => {
                if (currentPageIsHybrid) return;
              ...
              className={`rounded px-2.5 py-1 text-[0.625rem] font-bold uppercase tracking-wide transition disabled:cursor-not-allowed disabled:opacity-40 ${
                columnAdjustEnabled
                  ? "pdf-preview-bottom-actions__selected"
                  : "bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-[#30363d] dark:text-[#c9d1d9] dark:hover:bg-[#3d444d]"
              }`}
