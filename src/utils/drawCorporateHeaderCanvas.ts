...
export function drawCorporateHeaderCanvas(p: CorporateHeaderDrawParams): number {
  ...
  const ff = (field: HeaderFontFieldKey, page: 'first' | 'running' = 'first') =>
    fieldFontPt(field, sid, badge, page) * s
  const bodyHPt = style1BodyHeightPt(badge, sid)
  ...
  const primary = rgbCss(badge.primaryColor || '#0A1931')
  const accent = rgbCss(badge.accentColor || '#F34A2F')
  ...
  const rightSlots = resolveBannerRightSlots(badge)
  ...
  if (rightSlots.length > 0) {
    ...
    for (const { slot, top: topPt, h: slotHPt } of placed) {
      const slotTop = bodyY + topPt * s
      const slotH = slotHPt * s
      if (slot === 'examType' && shouldDrawExamTypeBoxContent(badge)) {
        const boxWPt = style1RightSlotWidthPt(slot, badge, maxBoxWPt)
        const boxW = boxWPt * s
        const boxX = p.pageWpx - p.mr - boxW - resolveExamTypeOffsetXPt(badge) * s
        drawExamTypeBoxFillCanvas(p.ctx, boxX, slotTop, boxW, slotH, badge, s)
        drawExamTypeBoxBorderCanvas(p.ctx, boxX, slotTop, boxW, slotH, badge, s)
        drawExamTypeTextInBox(
          p.ctx,
          boxX,
          slotTop,
        ...
        const boxW = style1RightSlotWidthPt(slot, badge) * s
  ...
  const topicSubTopicGap = (p.config.topicSubTopicGapPt ?? 3) * s
  ...
  return style1BannerBlockHeightPt(badge, sid)
}
...
export function drawStyle1RunningHeaderCanvas(
  ...
  const accent = rgbCss(p.config.accentColor || '#F34A2F')
