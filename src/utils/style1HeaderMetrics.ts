...
import { style1RightSlotsTotalHeightPt } from './bannerRightSlots'
...
function style1RightSideHeightPt(config: HeaderConfig): number {
  const stacked = style1RightSlotsTotalHeightPt(config)
  if (stacked > 0) return stacked
  // legacy tek mod (slots yoksa resolveBannerRightSlots zaten mode’dan doldurur)
 ...
 * Çağıran, mümkünse mergeHeaderBadgeConfig ile birleştirilmiş config vermeli —
 * ham kayıtlı eski rozet yükseklikleri gövdeyi şişirir.
 */
