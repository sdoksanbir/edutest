...
function PresetColorRow({ value, onChange, disabled, label }: SwatchRowProps) {
  return (
    <div className="mb-3">
...
export default function ExplanationCaptionModal({ open, onClose, question }: Props) {
  ...
  const [color, setColor] = useState(APP_COLOR_DEFAULT);
  ...
  useEffect(() => {
    ...
    setColor(question.explanation_caption_color ?? APP_COLOR_DEFAULT);
  ...
  const handleSave = () => {
    ...
    void setCaption(question.id, {
      ...
      explanation_caption_color: color || APP_COLOR_DEFAULT,
