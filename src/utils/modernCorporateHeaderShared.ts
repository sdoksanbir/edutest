/** Modern Kurumsal (Tema 1) — ortak ölçü ve renk sabitleri (Canvas + PDF) */

export const MM_TO_PT = 72 / 25.4

export const MODERN_PRIMARY = '#0A1931'
export const MODERN_ACCENT = '#F34A2F'
export const MODERN_GOLD = '#C59B27'
export const MODERN_GOLD_LIGHT = '#D4AF37'
export const MODERN_GOLD_DARK = '#9A7518'

export const MODERN_FONT_SANS = 'Inter, Montserrat, Helvetica, Arial'
export const MODERN_FONT_SERIF = 'Georgia, "Times New Roman", serif'

export const MODERN_FOOTER_RED_RATIO = 0.45
export const MODERN_BOX_BORDER_PT = 1.5
/** Logo–orta blok dikey ayırıcı */
export const MODERN_DIVIDER_V_PT = 1.2
/** Konu satırı yatay çizgileri + marka kutusu ayırıcı */
export const MODERN_TOPIC_LINE_PT = 1.0
/** Orta sütun kenarından konu çizgisine boşluk */
export const MODERN_TOPIC_COL_PAD_PT = 2
/** Konu metni ile dekoratif çizgi arası */
...
/** Ders adı yazısı — kutu içinde dikey kaydırma (pt; − yukarı, + aşağı) */
export const SUBJECT_PILL_TEXT_OFFSET_Y_DEFAULT_PT = -3
...
export const TOPIC_SUBTOPIC_GAP_DEFAULT_PT = 3
...
/** @deprecated use clampSubjectPillPadXPt / clampSubjectPillPadYPt */
export function clampSubjectPillPadPt(pt: number): number {
  return clampSubjectPillPadXPt(pt)
}

/** @deprecated use resolveSubjectPillPadXPt / resolveSubjectPillPadYPt */
export function resolveSubjectPillPadPt(config: {
  subjectPillPadPt?: number
  subjectPillPadXPt?: number
  subjectPillPadYPt?: number
}): number {
  return resolveSubjectPillPadXPt(config)
}
...
/** Konu satırı — orta sütun genişliğine yayılan sol/sağ çizgiler */
...
/** Slanted dual-color bar — left redRatio red, rest primary */
export function slantedBarGeometry(
  ml: number,
  contentW: number,
  stripeH: number,
  redRatio = MODERN_FOOTER_RED_RATIO,
): { redPath: [number, number][]; primaryPath: [number, number][] } {
