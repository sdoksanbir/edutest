...
type Props = {
  ...
  selectedFullWidthEnabled?: boolean;
  selectedFullWidthChecked?: boolean;
  onSelectedFullWidthChange?: (fullWidth: boolean) => void;
  /** Teşhis önerisi — otomatik değiştirmez */
  layoutRecommendationHint?: string | null;
};
...
function ScaleSliderField({
  ...
  const { tokens: t } = usePdfPreviewUi();
  /** pointercancel, pointerup’tan önce session’ı silmesin; lostcapture çift commit yapmasın */
  const gestureRef = useRef<"idle" | "active" | "done">("idle");

  const finishCommit = (pct: number) => {
    if (gestureRef.current !== "active") return;
    gestureRef.current = "done";
    onCommit(pct);
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <input
        ...
        onPointerUp={(e) => finishCommit(Number(e.currentTarget.value))}
        onLostPointerCapture={(e) => {
          // pointerup kaçarsa: hâlâ active ise commit et (cancel zaten done yapar)
          if (gestureRef.current === "active") {
            finishCommit(Number(e.currentTarget.value));
          }
        }}
...
export default function PageStructurePanel({
  ...
  selectedFullWidthEnabled = false,
  selectedFullWidthChecked = false,
  onSelectedFullWidthChange,
  layoutRecommendationHint = null,
}: Props) {
  const { tokens: t } = usePdfPreviewUi();
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {MARGIN_PRESETS.map((p) => (
              <SegBtn
                ...
                className="py-2"
                title={
                  p.topMm === p.leftMm && p.topMm === p.rightMm && p.topMm === p.bottomMm
                    ? `${p.label}: ${p.topMm / 10} cm her kenar`
                    : `${p.label}: sol ${p.leftMm / 10} cm, diğer ${p.topMm / 10} cm`
                }
              >
                {p.label}
              </SegBtn>
        ...
        <div className="pdf-preview-collapsible-section space-y-2.5">
          ...
          <ScaleSliderField
            ...
            hint={
              selectedQuestionScaleEnabled
                ? selectedQuestionScaleMaxPct < DISPLAY_SCALE_MAX_PCT
                  ? "%100 mevcut boyuttur; büyütünce sütun sınırına kadar gider, sığmazsa sonraki sütuna taşınır."
                  : "%100 = seçili soruların şu anki boyutu. Kaydırınca hepsi aynı oranda büyür/küçülür."
